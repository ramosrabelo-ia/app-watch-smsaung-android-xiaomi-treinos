...
public final class NotificationHelper {
    private static final String CHANNEL_ID = "treino_da_luana_v8";
    private static final int NOTIFICATION_ID = 8008;
    ...
    public static boolean show(Context context, int workout, int exercise) {
        ...
        PendingIntent openPending = PendingIntent.getActivity(
                context, 8000 + workout, open,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE
        );

        PendingIntent previous = action(context, WorkoutReceiver.ACTION_PREVIOUS, workout, exercise, 8100);
        PendingIntent next = action(context, WorkoutReceiver.ACTION_NEXT, workout, exercise, 8200);
        PendingIntent finish = action(context, WorkoutReceiver.ACTION_FINISH, workout, exercise, 8300);

        Notification.Builder builder = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O
                ? new Notification.Builder(context, CHANNEL_ID)
                : new Notification.Builder(context);

        builder.setSmallIcon(com.luanarabelo.treinodaluana.v8.R.drawable.ic_notification)
