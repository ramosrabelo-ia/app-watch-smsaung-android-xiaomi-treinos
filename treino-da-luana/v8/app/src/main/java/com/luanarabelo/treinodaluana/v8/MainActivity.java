...
public class MainActivity extends Activity {
    private static final int NOTIFICATION_PERMISSION_REQUEST = 808;
    ...
    private void openFromIntent(Intent intent) {
        ...
        if (workout >= 0 && workout < WorkoutData.NAMES.length) {
            showExercise(workout, Math.max(0, Math.min(exercise, WorkoutData.NAMES[workout].length - 1)));
        } else {
            showHome();
        }
    }
    ...
    private LinearLayout newScreen() {
        ...
        scroll.setOnApplyWindowInsetsListener((view, insets) -> {
            ...
            root.setPadding(dp(20), safeTop + dp(22), dp(20), safeBottom + dp(22));
            return insets;
        });
    ...
    private void showHome() {
        ...
        addHeader(root, "V8 // TREINO COMPLETO");

        TextView instruction = text("ESCOLHA O TREINO PARA VER TODOS OS EXERCÍCIOS", 14, GREEN);
    ...
    private void showExercise(int workout, int exercise) {
        ...
        addHeader(root, "V8 // TREINO " + WorkoutData.LETTERS[workout]);
        ...
        ExerciseImageView image = new ExerciseImageView(
                this,
                WorkoutData.IMAGE_TYPES[workout][exercise]
        );
        ...
        TextView reference = text("IMAGEM DE REFERÊNCIA DO MOVIMENTO", 11, Color.rgb(155, 166, 176));
        reference.setGravity(Gravity.CENTER);
        reference.setPadding(0, 0, 0, dp(16));
        root.addView(reference, fullWidth());
        ...
        LinearLayout.LayoutParams navButton = new LinearLayout.LayoutParams(0, dp(54), 1f);
        ...
        notify.setText(notificationActive ? "ATUALIZAR NO RELÓGIO" : "INICIAR NO RELÓGIO");
        notify.setTextSize(15);
        notify.setTextColor(BLACK);
        notify.setTypeface(Typeface.MONOSPACE, Typeface.BOLD);
        notify.setGravity(Gravity.CENTER);
        GradientDrawable startBackground = new GradientDrawable();
        startBackground.setColor(GREEN);
        startBackground.setCornerRadius(dp(2));
        notify.setBackground(startBackground);
        notify.setOnClickListener(view -> requestNotification(workout, exercise));
        LinearLayout.LayoutParams notifyParams = fullWidth();
        notifyParams.height = dp(58);
        notifyParams.setMargins(0, dp(12), 0, 0);
        root.addView(notify, notifyParams);

        status = text(
                notificationActive
                        ? "● RELÓGIO ATIVO // EXERCÍCIO SINCRONIZADO"
                        : "O RELÓGIO RECEBERÁ O EXERCÍCIO PELA NOTIFICAÇÃO",
                13,
                notificationActive ? GREEN : WHITE
        );
        status.setGravity(Gravity.CENTER);
        status.setTextAlignment(TextView.TEXT_ALIGNMENT_CENTER);
        status.setPadding(dp(10), dp(14), dp(10), 0);
        root.addView(status, fullWidth());
    }

    private void changeExercise(int delta) {
        int nextExercise = Math.max(
                0,
                Math.min(currentExercise + delta, WorkoutData.NAMES[currentWorkout].length - 1)
        );
    ...
    private void publishExercise(int workout, int exercise) {
        try {
            if (NotificationHelper.show(this, workout, exercise)) {
                ...
                Toast.makeText(this, "Exercício enviado para as notificações", Toast.LENGTH_SHORT).show();
            } else {
                status.setText("Não foi possível mostrar a notificação.");
            }
        } catch (Throwable error) {
            status.setText("Erro ao enviar a notificação.");
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (grantResults.length > 0
            ...
            status.setText("As notificações foram bloqueadas no celular.");
        }
        pendingWorkout = -1;
        pendingExercise = 0;
    }
